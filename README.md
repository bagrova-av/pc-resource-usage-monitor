...
#### Особенности:
- **CPU Load**: hассчитывается как разница между двумя снимками состояния `/proc/stat`. Это позволяет отображать мгновенную нагрузку, а не среднюю за всё время работы ОС.
- **RAM Info**: lля точности используется значение `MemAvailable` из `/proc/meminfo`, так как оно лучше отражает реальное количество свободной памяти в Linux, чем `MemFree` (учитывая кэш и буферы).
- **SVG Gauges**: dизуализация выполнена на SVG-дугах. Длина полоски управляется через CSS свойство `stroke-dashoffset`.
...
- Выводит прощальное сообщение в терминал.
...
- [nlohmann/json](https://github.com/nlohmann/json) — работа с JSON в C++.
